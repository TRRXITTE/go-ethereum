...
GETH_PATH="/home/traaitt/go-ethereum/build/bin/geth"
...
ETHERBASE="0x144a2763e425C80b88A31C2aE398F4e51C79A449"

# Step 1: Check if Geth is installed at the specified path and matches version
if [ ! -f "$GETH_PATH" ]; then
    echo "Geth not found at $GETH_PATH. Please ensure it is installed."
    exit 1
fi

GETH_INSTALLED_VERSION=$($GETH_PATH version | grep "Version: $GETH_VERSION" | wc -l)
...
$GETH_PATH --datadir "$DATADIR" init "$GENESIS_FILE"
if [ $? -eq 0 ]; then
...
ENODE=$($GETH_PATH --datadir "$DATADIR" --exec "admin.nodeInfo.enode" console 2>/dev/null | tr -d '"')
if [ -z "$ENODE" ]; then
